...
import br.com.futurodev.jwt_sample.exception.InvalidJwtAuthenticationException;
...
public class JwtAuthenticationProvider {
    ...
    public JwtAuthenticationProvider(@Value("${security.jwt.token.expires-length:3600000}") Long validityInMilliseconds,
                                     PersonRepository personRepository, @Value("${security.jwt.token.secret-key:secret}") String secretKey) {
    ...
    private String configureRefreshToken(String username, Date currentDate) {
        Date newExpiredAtWithRefreshToken = new Date(currentDate.getTime() + (this.validityInMilliseconds * 3)); // 3h
        return JWT.create()
                .withClaim("roles", new ArrayList<>())
                .withIssuedAt(currentDate)
